...
class LMark():
    ...
    Members:
        ...
        unit_length: The estimated grid square unit length that the mark is
            built with.
    """
    def __init__(self, polygon: open_mcr.geometry_utils.Polygon):
        """Create a new LMark. If the points don't form a valid LMark, raises a
        WrongShapeError."""
        if len(polygon) != 6:
            raise WrongShapeError("Incorrect number of points.")

        if not open_mcr.geometry_utils.all_approx_square(polygon):
            raise WrongShapeError("Corners are not square.")

        clockwise_polygon = open_mcr.geometry_utils.polygon_to_clockwise(polygon)
        side_lengths = open_mcr.geometry_utils.calc_side_lengths(clockwise_polygon)
        longest_sides_indexes = open_mcr.list_utils.find_greatest_value_indexes(
            side_lengths, n=2)

        if not open_mcr.list_utils.is_adjacent_indexes(side_lengths,
        ...
        unit_lengths = open_mcr.math_utils.divide_some(side_lengths,
                                              longest_sides_indexes, 2)
        if not open_mcr.math_utils.all_approx_equal(unit_lengths):
            raise WrongShapeError(
                "Longest sides are not twice the length of the other sides.")

        self.polygon = open_mcr.list_utils.arrange_index_to_first(
            clockwise_polygon,
            open_mcr.list_utils.determine_which_is_next(polygon,
                                               *longest_sides_indexes))
        self.unit_length = open_mcr.math_utils.mean(unit_lengths)
...
class SquareMark:
    ...
    """
    def __init__(self,
                 polygon: open_mcr.geometry_utils.Polygon,
        ...
        side_lengths = open_mcr.geometry_utils.calc_side_lengths(polygon)
        if not open_mcr.math_utils.all_approx_equal(side_lengths, target_size):
            raise WrongShapeError(
                "Side lengths are not equal or too far from target_size.")

        clockwise = open_mcr.geometry_utils.polygon_to_clockwise(polygon)
        if clockwise[0] is polygon[0]:
            ...
            self.polygon = open_mcr.list_utils.arrange_index_to_first(
                clockwise,
                len(clockwise) - 1)
        self.unit_length = open_mcr.math_utils.mean(side_lengths)


def find_corner_marks(image: np.ndarray,
                      save_path: typing.Optional[pathlib.PurePath] = None
                      ) -> open_mcr.geometry_utils.Polygon:

    all_polygons: typing.List[
        open_mcr.geometry_utils.Polygon] = open_mcr.image_utils.find_polygons(
    ...
    # complexity.
    hexagons: typing.List[open_mcr.geometry_utils.Polygon] = []
    quadrilaterals: typing.List[open_mcr.geometry_utils.Polygon] = []
    ...
    if save_path:
        open_mcr.image_utils.draw_polygons(image, hexagons, save_path / "all_hexagons.jpg")
        open_mcr.image_utils.draw_polygons(image, quadrilaterals, save_path / "all_quadrilaterals.jpg")
    ...
    for i in range(len(hexagons)):
        ...
        if save_path:
            ...
            nominal_poly_new_basis = [
                open_mcr.geometry_utils.Point(0.0, 0.0),
                open_mcr.geometry_utils.Point(nominal_to_right_side, 0.0),
                open_mcr.geometry_utils.Point(nominal_to_right_side, nominal_to_bottom),
                open_mcr.geometry_utils.Point(0.0, nominal_to_bottom)
            ]
            # Boxes within which corner centroids can be found
            corner_tolerance_polys_new_basis = [
                [
                    open_mcr.geometry_utils.Point(x + x_tolerance, y - y_tolerance),
                    open_mcr.geometry_utils.Point(x + x_tolerance, y + y_tolerance),
                    open_mcr.geometry_utils.Point(x - x_tolerance, y + y_tolerance),
                    open_mcr.geometry_utils.Point(x - x_tolerance, y - y_tolerance)
                ] for [x, y] in [
            ...
            open_mcr.image_utils.draw_polygons(
                image,
                polys,
        ...
        for quadrilateral in quadrilaterals:
            try:
                ...
                continue
            centroid = open_mcr.geometry_utils.guess_centroid(square.polygon)
            centroid_new_basis = basis_transformer.to_basis(centroid)

            if open_mcr.math_utils.is_within_tolerance(
                    centroid_new_basis.x, nominal_to_right_side,
                    x_tolerance) and open_mcr.math_utils.is_within_tolerance(
                        centroid_new_basis.y, 0.5, y_tolerance):
                top_right_squares.append(square)
            elif open_mcr.math_utils.is_within_tolerance(
        ...
        top_right_corner = open_mcr.geometry_utils.get_corner_wrt_basis(
            top_right_squares[0].polygon, open_mcr.geometry_utils.Corner.TR, basis_transformer)
        bottom_right_corner = open_mcr.geometry_utils.get_corner_wrt_basis(
            bottom_right_squares[0].polygon, open_mcr.geometry_utils.Corner.BR, basis_transformer)
        bottom_left_corner = open_mcr.geometry_utils.get_corner_wrt_basis(
            bottom_left_squares[0].polygon, open_mcr.geometry_utils.Corner.BL, basis_transformer)
        ...
        if save_path:
            open_mcr.image_utils.draw_polygons(image, [grid_corners], save_path / "grid_limits.jpg")
